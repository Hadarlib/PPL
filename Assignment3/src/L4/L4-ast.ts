...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    (params) => 
    isEmpty(params) ? makeFailure("trace missing arguments") :
   //isString(params[0]) ? mapv( parseL4 ( params[0] ), (val: VarRef) => makeTraceExp( val )) :
   //makeFailure("non valid")
   isString(params[0]) ?makeOk( makeTraceExp(makeVarRef( params[0]))):
   makeFailure("non valid");
