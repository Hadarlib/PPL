...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
            theGlobalEnv, Env, FBinding, isFBinding, getFBindingVal, getFBindingVar } from "./L4-env-box";
...
// HW3
const evalTraceExp = (exp: TraceExp): Result<void> => { //convert closure to tracedClosure
    return bind(applyEnvBdg(theGlobalEnv ,exp.var.var), (fb : FBinding) => 
    isClosure(getFBindingVal(fb)) ?
        makeOk(setFBinding(fb , makeTracedClosure( getFBindingVal(fb) as Closure, getFBindingVar(fb)))):
        makeFailure<void>("cannot be traced"));
}
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    printPreTrace(proc.traceName, args, proc.counter);
    proc.counter = proc.counter + 1;
    
    let val = applyClosure(proc.traceClosure , args);
    
    proc.counter = proc.counter - 1;
    isOk(val) ? printPostTrace( val.value, proc.counter):
    makeFailure("cannot be traced");
    return val;
}
