...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         isDefineTypeExp, isTypeCaseExp, DefineTypeExp, TypeCaseExp, CaseExp, makeLitExp } from "./L5-ast";
...
const isSubType = (te1: TExp, te2: TExp, p: Program): boolean =>{
    if(isAnyTExp(te2)) //if te2 is any, we can accept anything
        return true;

    //relevent TExp are: userDef, userDefName, Record
    //making sure t1, t2 are record/userDef    
    let t1 ;
    let t2 ; 
    if(isUserDefinedNameTExp(te1)){
        let res = getTypeByName(te1.typeName, p);
        if(isOk(res))
            t1 = res.value;
    }
    if(isUserDefinedNameTExp(te2)){
        let res = getTypeByName(te2.typeName, p);
        if(isOk(res))
            t2 = res.value;
    }
    if(t1 !== undefined && t2 !== undefined){
    //checking subTypes, only relevant if t1 is record and t2 is userDef
        if(isRecord(t1) && isUserDefinedTExp(t2)){
            let parents = getRecordParents(t1.typeName, p);
            for(let par of parents){
                if(par.typeName === t2.typeName)
                    return true;
            }
        } 
    }
...
export const initTEnv = (p: Program): TEnv =>{
    ...
    let userDefinedArr = getTypeDefinitions(p);
    let userDefinedVars = new Array();
    let userDefinedTypes = new Array();
    for(let userDef of userDefinedArr){
        userDefinedVars.push(userDef.typeName.concat('?'));
        userDefinedTypes.push(makeProcTExp([makeAnyTExp()],makeBoolTExp()));
    }
    tenv = makeExtendTEnv(userDefinedVars, userDefinedTypes, tenv);

    let defineNames = userDefinedArr.map((def)=> def.typeName);
    tenv = makeExtendTEnv(defineNames, userDefinedArr, tenv);

    let recordsArr = getRecords(p);
    let recordsVars = new Array();
    let recordsTypes = new Array();
    for(let rec of recordsArr){
        recordsVars.push(rec.typeName.concat('?'));
        recordsTypes.push(makeProcTExp([makeAnyTExp()],makeBoolTExp()));
        recordsVars.push("make-".concat(rec.typeName));
        let fieldTypes = new Array();
        for(let field of rec.fields){

            fieldTypes.push(field.te);
        }
        recordsTypes.push(makeProcTExp(fieldTypes, rec));
    }
    tenv = makeExtendTEnv(recordsVars, recordsTypes, tenv);

    let recordsNames = recordsArr.map((rec)=> rec.typeName);
    tenv = makeExtendTEnv(recordsNames, recordsArr, tenv);
    
    let defineArr = getDefinitions(p);
    let defineVars = new Array();
    let defineTypes = new Array();
    for(let def of defineArr){
        defineVars.push(def.var.var);
        defineTypes.push(def.var.texp);
    }
    tenv = makeExtendTEnv(defineVars, defineTypes, tenv);
    return tenv;
}
...
const checkUserDefinedTypes = (p: Program): Result<true> =>{
    ...
    let recordsArr = getRecords(p);
    while(!isEmpty(recordsArr)){
        let currRec = recordsArr.pop();
        if(isRecord(currRec)){
            for(let i = 0; i < recordsArr.length ; i++){
                let tmpRec = recordsArr[i];
                if(currRec.typeName === tmpRec.typeName){
                    if(currRec.fields.length === tmpRec.fields.length){
                        for(let j = 0; j < currRec.fields.length ; j++){
                            if(!(currRec.fields[j].fieldName === tmpRec.fields[j].fieldName &&
                                currRec.fields[j].te === tmpRec.fields[j].te)){
                                    return makeFailure<true>("No matching fields");
                                }
                        }
                    }
                    else
            ...
            recordsArr = recordsArr.filter((rec) => currRec?.typeName !== rec.typeName);
        }
    } 

    //check for existance of basecase in each define type in the program
    let defineTypeArr = getTypeDefinitions(p);
    for(let def of defineTypeArr){
        let baseFound = false; 
        for(let defRec of def.records){
            for(let i = 0; i < defRec.fields.length ; i++){
                if(isUserDefinedTExp(defRec.fields[i].te))
                    break;
                else if(i === defRec.fields.length-1 && !isUserDefinedTExp(defRec.fields[i].te))
                    baseFound = true;
            }
            if(baseFound)
                break;
        ...
        if(!baseFound)
            return makeFailure<true>("No base case");
    }
    return makeOk(true);
...
const checkTypeCase = (tc: TypeCaseExp, p: Program): Result<true> => {
    ...
    // (in any order)
    let cases = tc.cases;
    let expTypes : UDTExp []= new Array();
    for(let cas of cases){
        let casType = getTypeByName(cas.typeName, p);
        if(isOk(casType))
            expTypes.push(casType.value);
    }
    let userDefine = getUserDefinedTypeByName(tc.typeName,p);
    if(!isOk(userDefine)){
        let cover = checkCoverType(expTypes , p);
        if(isOk(cover))
            userDefine = makeOk(getRecordParents(tc.typeName,p)[0]);
        else
            return makeFailure<true>("No valid type");
    }

    
    if(isOk(userDefine)){
        let records = userDefine.value.records;
        if(records.length === cases.length){
            for(let rec of records){
                let found = false;
                for(let cas of cases){
                    if(rec.typeName === cas.typeName && rec.fields.length === cas.varDecls.length){
                        found = true;
                        break;
                    }
                }
                if(!found)
                    return makeFailure<true>("Not all the cases appear");
            }
            return makeOk(true);
        }
        else{
            return makeFailure<true>("Number of records not equals to number of cases");
        }

    }
    else
        return makeFailure<true>("Error");
}
...
export const L51typeofProgram = (concreteExp: string): Result<string> =>
    bind(parseL51(concreteExp), (p: Program) =>
        ...
        bind(typeofExp(p, initTEnv(p), p), unparseTExp): makeFailure("error"));
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv, p: Program): Result<TExp> => {//? maybe only constraint 4
    ...
    const constraint2 = bind(thenTE, (thenTE: TExp) =>
                            bind(altTE, (altTE: TExp) =>
                                checkEqualType(thenTE, altTE, ifExp, p)));//check if thenTE equal or subtype of altTE
    if(isOk(constraint2))
        return bind(constraint1, (_c1) => constraint2);
    else{
        const constraint3 = bind(thenTE, (thenTE: TExp) =>
                            bind(altTE, (altTE: TExp) =>
                                checkEqualType(altTE, thenTE, ifExp, p)));//check if altTE is subtype of thenTE
        if(isOk(constraint3))
            return bind(constraint1, (_c1) => constraint3);
        else{
            const constraint4 = bind(thenTE, (thenTE: TExp) => //return the most specifict common type of alt and then
                                bind(altTE, (altTE: TExp) =>
                                checkCoverType([thenTE, altTE], p)));
            return bind(constraint1, (_c1) => constraint4);
        }
    }
};
...
export const typeofDefineType = (exp: DefineTypeExp, _tenv: TEnv, _p: Program): Result<TExp> =>{
        const v = exp.typeName;
        const texp = exp.udType;
        const val = texp.records;
        //const tenvVal = makeExtendTEnv([v], [texp], _tenv);
        return makeOk(makeVoidTExp());
}

// TODO L51
export const typeofSet = (exp: SetExp, _tenv: TEnv, _p: Program): Result<TExp> =>{
    let varTypeRes = applyTEnv(_tenv, exp.var.var); 
    if(isOk(varTypeRes)){
        let varType = varTypeRes.value;
        let expTypeRes = typeofExp(exp.val, _tenv, _p);
        if(isOk(expTypeRes)){
            let expType = expTypeRes.value;
            if(isOk(checkEqualType(expType, varType, exp, _p)))
                return makeOk(makeVoidTExp());
            else
                return makeFailure("Cannot set because of typing rule");
        }
        else
            return makeFailure("error");
    }
    else
        return makeFailure("Var doesn't exist");
...
export const typeofLit = (exp: LitExp, _tenv: TEnv, _p: Program): Result<TExp> =>
    ...
    makeFailure<TExp>("Not type of lit expression");
...
export const typeofTypeCase = (exp: TypeCaseExp, tenv: TEnv, p: Program): Result<TExp> => {
    if(isOk(checkTypeCase(exp, p))){
        let typesArr : TExp[] = new Array();
        for(let cas of exp.cases){
            let rec = getRecordByName(cas.typeName ,p);
            if(isOk(rec)){
                let typeCas = typeofExps(cas.body,makeExtendTEnv(cas.varDecls.map(vardecl => vardecl.var),
                rec.value.fields.map(field => field.te), tenv), p);
                if(isOk(typeCas))
                    typesArr.push(typeCas.value);
                else
                    return makeFailure("error");
            }
            else 
        ...
        return checkCoverType(typesArr, p);
    }
    else{
        return makeFailure("Not correct semmantics for type-case");
    }
}
