...
export const isAtomicTExp = (x: any): x is AtomicTExp =>
    isNumTExp(x) || isBoolTExp(x) || isStrTExp(x) || isVoidTExp(x) || isUserDefinedNameTExp(x) || isAnyTExp(x) || isSymbolTExp(x) || isPairTExp(x); // L51
...
export type LitTexp = SymbolTExp | PairTExp; //lit type - we added
export const isLitTexp = (x: any): x is LitTexp => isSymbolTExp(x) || isPairTExp(x);


export type SymbolTExp = { tag: "SymbolTExp", val?: SymbolSExp };// symbol type - we added
export const makeSymbolTExp = (x?: SymbolSExp): SymbolTExp => ({tag: "SymbolTExp", val: x});
export const isSymbolTExp = (x: any): x is SymbolTExp => x.tag === "SymbolTExp";

export type PairTExp = { tag: "PairTExp" }; // pair type - we added
