...
(define pipe$
  (lambda (fs c)
    (if (empty? (cdr fs))
        ...
        (pipe$ (cdr fs)
                (lambda (pipe-res)
                  (compose$ (car fs) pipe-res c))))))


(define compose$
  (lambda (f$ g$ c)
   (c(lambda (x c2)
       (f$ x (lambda (f-res) (g$ f-res c2)))))))
...
(define reduce-prim$
  (lambda (reducer init lst cont)
    (if (empty? lst)
        (cont init)
        (reduce-prim$ reducer init (cdr lst) (lambda (res)(cont(reducer res (car lst))))))))
...
(define reduce-user$
  (lambda (reducer$ init lst cont)
    (if (empty? lst)
        (cont init)
        (reduce-user$ reducer$ init (cdr lst) (lambda (res)(reducer$ res (car lst) cont))))))
...
(define take-while
  (lambda (lz-lst pred)
    (cond ((empty-lzl? lz-lst)lz-lst)
        ((pred (head lz-lst))(cons (head lz-lst)(take-while (tail lz-lst) pred)))
        (else '()))))
...
(define take-while-lzl
  (lambda (lz-lst pred)
    (cond ((empty-lzl? lz-lst)lz-lst)
        ((pred (head lz-lst))(cons-lzl (head lz-lst)(lambda () (take-while-lzl (tail lz-lst) pred))))
        (else '()))))

;;; Q2.d
; Signature: reduce-lzl(reducer, init, lzl)
; Type: [(T1*T2->T2)* T2 * LzL(T1) -> T2]
